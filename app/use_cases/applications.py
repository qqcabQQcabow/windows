...
from ..infrastructure.log_utils import logger
...
def change_driver(causer: JWTPayload, data: ChangeApplicationDriver) -> Optional[str]:
    try:
        ...
        success = db_applications.change_driver(
            data.application_id, data.new_driver_login
        )
        if not success:
...
def retrieve_all_states(
    causer: JWTPayload, application_id: int
) -> Tuple[list[dict[Any, Any]], Optional[str]]:
    try:
        if causer.role == RoleEnum.LOGIST and not db_applications.contains_logist(
            application_id, causer.login
        ):
            return [], f"Нет доступа"
        if causer.role == RoleEnum.DRIVER and not db_applications.contains_driver(
            application_id, causer.login
        ):
            return [], f"Нет доступа"
...
def init_state(causer: JWTPayload, new_state: ApplicationState) -> Optional[str]:
    try:
        ...
        if not db_applications.active_with_driver(causer.login):
            return "Нельзя изменить эту заявку"

        if not db_applications.can_init_state(new_state):
            return "Нельзя изменить этот статус"
